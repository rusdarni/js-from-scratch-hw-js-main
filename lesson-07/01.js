...
const changeImageButtonPrev = document.querySelector('#prev-button')
const changeImageButtonNext = document.querySelector('#next-button')
let index = 0
changeImageButtonPrev.addEventListener('click', function() {
  const images = document.querySelector('#web-tech-image')
  index = index - 1
  if(index<0){
    index = index+3
  }
  images.setAttribute('src', WEB_TECH_IMAGES[index])
})
changeImageButtonNext.addEventListener('click', function() {
  const images = document.querySelector('#web-tech-image')
  index = index + 1
  if (index>2){
    index = index-3
